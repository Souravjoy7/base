...
struct WorkerArgs {
    ...
    /// Base consensus node RPC URL. Required for `ZK_BACKEND=dry_run`, `cluster`, or `network`.
    #[arg(
        long,
    ...
    /// L1 execution node RPC URL. Required for `ZK_BACKEND=dry_run`, `cluster`, or `network`.
    #[arg(
        long,
        env = "L1_NODE_ADDRESS",
        required_if_eq_any([("backend", "dry-run"), ("backend", "cluster"), ("backend", "network")])
    ...
    /// L1 beacon node RPC URL. Required for `ZK_BACKEND=dry_run`, `cluster`, or `network`.
    #[arg(
        long,
    ...
    /// L2 execution node RPC URL. Required for `ZK_BACKEND=dry_run`, `cluster`, or `network`.
    #[arg(
        long,
        env = "L2_NODE_ADDRESS",
        required_if_eq_any([("backend", "dry-run"), ("backend", "cluster"), ("backend", "network")])
...
impl WorkerArgs {
    fn backend_config(&self) -> eyre::Result<SuccinctZkBackendConfig> {
        match self.backend {
            ...
            ZkBackendArg::DryRun => Ok(SuccinctZkBackendConfig::DryRun(self.rpc_config()?)),
